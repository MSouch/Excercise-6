...
const ExerciseForm=({exercise,onComplete})=> {
  ...
  useEffect(()=> {
    // Auto-save every 30 seconds only while editing and not completed
    const interval=setInterval(()=> {
      if (!completed && isEditing && Object.keys(formData).length > 0) {
        handleAutoSave()
      }
    },30000)
  ...
  const handleAutoSave=()=> {
    if (!user || loading || completed) return
  ...
  setSaved(true)
  // Hide saved banner shortly after when editing stops
  ...
  const handleSubmit=async (e)=> {
    ...
    try {
      ...
      // Do not auto-progress; reveal model answer immediately and enable proceed explicitly
